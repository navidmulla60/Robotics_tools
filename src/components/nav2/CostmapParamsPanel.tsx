...
function CommonCostmapFields({ params, onChange }: { params: CostmapCommonParams; onChange: (p: CostmapCommonParams) => void }) {
  ...
  return (
    <>
      <NumberField label="resolution (m)" value={params.resolution} onChange={(v) => set('resolution', v)} decimals={3} />
      <NumberField label="update_frequency (Hz)" value={params.update_frequency} onChange={(v) => set('update_frequency', v)} decimals={2} />
      <NumberField label="publish_frequency (Hz)" value={params.publish_frequency} onChange={(v) => set('publish_frequency', v)} decimals={2} />
      ...
      {params.rolling_window && (
        <>
          <NumberField label="width (m)" value={params.width} onChange={(v) => set('width', v)} decimals={1} />
          <NumberField label="height (m)" value={params.height} onChange={(v) => set('height', v)} decimals={1} />
        </>
      )}
    </>
  );
      ...
      <div className="space-y-3">
        <div className="border-t border-neutral-100 pt-3 first:border-t-0 first:pt-0 dark:border-neutral-800">
          ...
          <div className="grid grid-cols-2 gap-3 sm:grid-cols-3">
            <NumberField label="inflation_radius (m)" value={inflation.inflation_radius} onChange={(v) => setInflation('inflation_radius', Math.max(0, v))} decimals={3} />
            <NumberField label="cost_scaling_factor" value={inflation.cost_scaling_factor} onChange={(v) => setInflation('cost_scaling_factor', Math.max(0.01, v))} decimals={2} />
        ...
        <Section title="Obstacle layer">
          ...
          <NumberField label="min_obstacle_height (m)" value={obstacle.min_obstacle_height} onChange={(v) => setObstacle('min_obstacle_height', v)} decimals={2} />
          <NumberField label="max_obstacle_height (m)" value={obstacle.max_obstacle_height} onChange={(v) => setObstacle('max_obstacle_height', v)} decimals={2} />
          <NumberField label="obstacle_min_range (m)" value={obstacle.obstacle_min_range} onChange={(v) => setObstacle('obstacle_min_range', v)} decimals={2} />
          <NumberField label="obstacle_max_range (m)" value={obstacle.obstacle_max_range} onChange={(v) => setObstacle('obstacle_max_range', v)} decimals={2} />
          <NumberField label="raytrace_min_range (m)" value={obstacle.raytrace_min_range} onChange={(v) => setObstacle('raytrace_min_range', v)} decimals={2} />
          <NumberField label="raytrace_max_range (m)" value={obstacle.raytrace_max_range} onChange={(v) => setObstacle('raytrace_max_range', v)} decimals={2} />
        </Section>
